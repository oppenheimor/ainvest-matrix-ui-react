...
interface YearCalendarProps extends Omit<BaseDatePickerProps, 'viewMode'> {
  startYear: number;
  selectedDate?: Date;
}
...
export const YearCalendar: React.FC<YearCalendarProps> = ({
  ...
  const handleYearClick = (year: number) => {
    ...
    // 创建新的日期对象，保留当前选中日期的月和日，只改变年份
    // 如果没有当前选中日期，则使用当前日期的月和日
    let newDate;
    if (selectedDate) {
      // 保留选中日期的月和日，只改变年份
      newDate = dayjs(selectedDate).tz(timezone).year(year).toDate();
    } else {
      // 如果没有选中日期，使用当前日期的月和日
  ...
  return (
    <div className='date-picker-year-calendar'>
      <div className='date-picker-years'>
        <div className='date-picker-years-grid'>
          {years.map((year) => {
            // 判断当前年份是否被选中
            const isSelected =
              selectedDate && dayjs(selectedDate).tz(timezone).year() === year.value;

            const isDisabled = isYearDisabled(year.value);

            // 确定年份应用的CSS类
            const yearClasses = ['date-picker-year', isSelected ? 'selected' : '', isDisabled ? 'disabled' : '']
              .filter(Boolean)
              .join(' ');
            ...
            return (
              ...
              >
                <span className='date-picker-year-text'>{year.label}</span>
              </div>
            );
          })}
        </div>
