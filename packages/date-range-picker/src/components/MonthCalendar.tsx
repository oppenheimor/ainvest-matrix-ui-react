...
interface MonthCalendarProps extends BaseDateRangePickerProps {
  currentYear: number;
  selectedDate?: Date;
}
...
export const MonthCalendar: React.FC<MonthCalendarProps> = ({
  ...
  return (
    <div className='widget-date-range-picker-month-calendar'>
      <div className='widget-date-range-picker-year-title'>{currentYear}</div>
      <div className='widget-date-range-picker-months'>
        <div className='widget-date-range-picker-months-grid'>
          {months.map((month) => {
            // 同时考虑年份和月份进行选中判断
            const isSelected =
              selectedDate &&
            ...
            // 确定月份应用的CSS类
            const monthClasses = ['widget-date-range-picker-month', isSelected ? 'selected' : '', isDisabled ? 'disabled' : '']
              .filter(Boolean)
              .join(' ');
            ...
            return (
              ...
              >
                <span className='widget-date-range-picker-month-text'>{month.label}</span>
              </div>
            );
          })}
        </div>
